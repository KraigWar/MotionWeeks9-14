...
public class GunshipController : MonoBehaviour
{
    ...
    private void FireCannonball(Vector3 direction, Transform cannon)
    {
         ...
         GameObject canonBall =  Instantiate(cannonballPrefab, cannon.position, Quaternion.identity);

         Rigidbody2D canonbody2D = canonBall.GetComponent<Rigidbody2D>();

         canonbody2D.AddForce(direction * cannonballForce, ForceMode2D.Impulse);
